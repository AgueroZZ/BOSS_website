...
simulate_once <- function(seed){
  ...
  for (i in 1:length(eval_num)) {
    ...
    ff$fn <- function(x)
      as.numeric(
        surrogate(
          ...
          choice_cov = square_exp_cov_generator_nd(
            length_scale = result_ad$length_scale,
            signal_var = result_ad$signal_var
          )
        )
      )
    x_vals <- (seq(
    ...
    post_x <- data.frame(y = x_vals, pos = exp(fn_vals - lognormal_const))
    BO_result_list[[i]] <- data.frame(x = (lower + x_vals * (upper - lower)),
...
mean_result <- all_result %>% group_by(eval_num) %>%
  summarise(KL = mean(KL), KS = mean(KS))
...
plot(mean_result$eval_num, mean_result$KL, type = "o", 
     ylab = "KL", xlab = "eval number: B",
     # log = "y",
     # ylim = c(min(lower_result$KL), max(upper_result$KL)),
     cex.lab = 1.0, cex.axis = 1.0)
polygon(c(mean_result$eval_num, rev(mean_result$eval_num)), 
        c(lower_result$KL, rev(upper_result$KL)), 
        col = rgb(0.2, 0.2, 0.2, 0.2), border = NA)

plot(mean_result$eval_num, mean_result$KS, type = "o",
     ylab = "KS", xlab = "eval number: B", 
     # log = "y",
     # ylim = c(min(lower_result$KS), max(upper_result$KS)),
     cex.lab = 1.0, cex.axis = 1.0)
polygon(c(mean_result$eval_num, rev(mean_result$eval_num)), 
