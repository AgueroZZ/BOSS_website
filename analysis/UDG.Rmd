...
#tikz(file = "joint_post_R_n_inlabru.tex", standAlone=T, width = 4, height = 3)
ggplot(inla.joint.samps, aes(R, n)) + stat_density_2d(
  geom = "raster",
  aes(fill = after_stat(density)), n = 300,
  contour = FALSE) +
  geom_point(data = data.frame(R = R_fun_1(fit$summary.fixed$mode[2]), n = alpha_fun_1(fit$summary.fixed$mode[3])), color = 'red', shape = 1, size =0.5) + scale_fill_viridis_c(name = 'Density') + theme_minimal() + xlab('$R$') + ylab('$n$') + xlim(c(2, 12)) + ylim(c(0.15, 2))
#dev.off()
...
#tikz(file = "joint_post_R_n.tex", standAlone=T, width = 4, height = 3)
