...
We also need to note that the above test only showcases the performance of BO when the true posterior is a multivariate Gaussian. We have observed that if there is deviation from normality, then convergence would take even longer for, especially for higher dimension. Under the most extreme cases where certain parameters are almost degenerate with respect to each other, BO can almost entirely fail.
