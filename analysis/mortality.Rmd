...
Although the BOSS surrogate does not provide perfectly accurate estimate for the second moment, the first moment is extremely close to the exact grid. 
Considering that we only need the mode to perform empirical Bayes in this example, the BOSS surrogate is a good approximation to the exact grid. 
...
The relative difference is only `r abs(exact_grid_result_BG$x[which.max(exact_grid_result_BG$exact_vals)] - my_alpha_BG)/abs(exact_grid_result_BG$x[which.max(exact_grid_result_BG$exact_vals)])`.
