...
```{r, echo = F}
set.seed(123)
res_opt_modal <- BOSS_modal(eval_func, update_step = 5, max_iter = 100, D = 2,
...
```{r, echo = F}
set.seed(123)
res_opt_aghq <- BOSS_aghq(eval_func, update_step = 5, max_iter = 100, D = 2,
                      lower = rep(0.1, 2), upper = c(5, 4),
                      noise_var = 1e-6,
...
From the above results, it is clear that BOSS is much better at depicting the joint posterior distribution than `inlabru`.
